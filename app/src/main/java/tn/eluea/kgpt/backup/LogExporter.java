...
public class LogExporter {
    ...
    private void addXposedLogs(ZipOutputStream zos) throws IOException {
        ...
        String xposedLogs;
        if (hasRootAccess) {
            xposedLogs = executeRootCommand("logcat -d -v time *:S XposedBridge:V EdXposed:V LSPosed:V KGPT:V KeyboardGPT:V");
        } else {
            xposedLogs = executeCommand("logcat -d -v time *:S XposedBridge:V EdXposed:V LSPosed:V KGPT:V KeyboardGPT:V");
        }
    ...
    private void addBootLogs(ZipOutputStream zos) throws IOException {
        ...
        if (hasRootAccess) {
            ...
            String lastKmsg = executeRootCommand("cat /proc/last_kmsg 2>/dev/null || cat /sys/fs/pstore/console-ramoops 2>/dev/null || echo 'Not available'");
    ...
    private void addCrashLogs(ZipOutputStream zos) throws IOException {
        ...
        logs.append("\n\n=== ANR Traces ===\n");
        if (hasRootAccess) {
            String anrTraces = executeRootCommand("cat /data/anr/traces.txt 2>/dev/null || echo 'No ANR traces found or permission denied'");
