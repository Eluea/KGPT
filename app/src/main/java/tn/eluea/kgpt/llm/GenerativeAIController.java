...
public class GenerativeAIController implements ConfigChangeListener {
    ...
    private void updateInternetProvider(Boolean enableExternalInternet) {
        // Use ExternalInternetProvider if enabled or default behavior requires it
        // For now, we enable it by default to support offline keyboards
        boolean useExternal = enableExternalInternet != null ? enableExternalInternet : true;

        if (useExternal) {
            tn.eluea.kgpt.util.Logger.log("Using ExternalInternetProvider for offline support");
            if (mExternalClient == null) {
                mExternalClient = new ExternalInternetProvider(mInteractor.getContext());
            }
