...
public class TextActionsFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getActivity() instanceof MainActivity) {
            ((MainActivity) getActivity()).setDockAction(getString(R.string.action_add_custom), R.drawable.ic_add,
                    v -> {
    ...
    private void populateActionsList() {
        ...
        for (TextAction action : TextAction.values()) {
            ...
            title.setText(action.getLabel(requireContext()));
            subtitle.setText(action.getLabel(requireContext()));
            ...
            switchAction.setOnCheckedChangeListener((buttonView, isChecked) -> {
                if (isChecked && action == TextAction.DOWNLOAD && !DownloaderEngine.getInstance().isCoreInstalled(requireContext())) {
            ...
            view.setOnClickListener(v -> {
                if (action == TextAction.DOWNLOAD && !DownloaderEngine.getInstance().isCoreInstalled(requireContext())) {
                    new CoreUpdateBottomSheet(requireContext()).show();
                    return;
                }
        ...
        for (tn.eluea.kgpt.features.textactions.domain.CustomTextAction action : customActions) {
            ...
            subtitle.setText(R.string.label_custom_action);
    ...
    private void showAddCustomActionDialog(TextActionManager actionManager) {
        ...
        btnSave.setOnClickListener(v -> {
            ...
            if (name.isEmpty() || prompt.isEmpty()) {
                if (getView() != null)
                    Snackbar.make(getView(), R.string.error_missing_name_prompt, Snackbar.LENGTH_SHORT).show();
                return;
            }
    ...
    private void showEditPromptDialog(TextAction action, TextActionManager actionManager) {
        ...
        title.setText(getString(R.string.edit_prompt_title, action.getLabel(requireContext())));
    ...
    private Set<TextAction> decodeEnabledActions(String json) {
        ...
        if (!actions.isEmpty() && !actions.contains(TextAction.DOWNLOAD)) {
            boolean explicitlyDisabled = requireContext().getSharedPreferences("keyboard_gpt", android.content.Context.MODE_PRIVATE)
